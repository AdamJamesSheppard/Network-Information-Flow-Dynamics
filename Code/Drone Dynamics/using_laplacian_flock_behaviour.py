...
def update_dynamics_with_laplacian(L, drones, alpha):
    for i, drone in enumerate(drones):
        laplacian_force = np.zeros(2)
        for j, other in enumerate(drones):
            if i != j:
                laplacian_force += L[i, j] * (other.position - drone.position)
        drone.velocity += alpha * laplacian_force
...
def animate(frame):
    ...
    # Update drone dynamics using Laplacian
    update_dynamics_with_laplacian(L, drones, alpha=0.01)
...
ani.save('flock_behaviour.mp4', writer='ffmpeg')
