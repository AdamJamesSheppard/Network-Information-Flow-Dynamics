...
def animate(frame):
    global drones, G, bounds, ax1, ax2, source_pos, target_pos
    ...
    # Draw positions only
    for drone in drones:
        ax2.plot(drone.position[0], drone.position[1], 'bo' if not drone.information else 'go')
    ax2.scatter(*source_pos, color='red', s=200, marker='o', label='Source')
    ax2.scatter(*target_pos, color='black', s=200, marker='x', label='Target')
    ax2.set_xlim(bounds[0][0], bounds[1][0])
    ax2.set_ylim(bounds[0][1], bounds[1][1])
    ax2.set_title(f"Drone Positions - Frame {frame}")
    ax2.legend()
...
ani.save('drone_swarm_simulation.mp4', writer='ffmpeg')
