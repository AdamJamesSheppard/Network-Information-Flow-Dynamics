...
def fit_logistic_curve(informed_counts, steps):
    ...
    try:
        popt, _ = curve_fit(logistic_function, t, informed_counts, p0=[L, 0.1, steps/2], maxfev=10000)
    except RuntimeError as e:
        print(f"Error fitting curve: {e}")
        popt = [L, 0.1, steps/2]  # Default parameters if fitting fails
    return popt
...
avg_connections = 3
initial_informed = 10
infection_prob = 0.1  # Increased infection probability
steps = 30
n_future_steps = 5
...
predicted_state = logistic_function(0, L, k, t0)
predicted_cov = 1
